...
function fetchInfraAndBeMode() {
  ...
  while (retryCount < maxRetries && !success) {
    let response = http.get('https://infra.antrein10.cloud');
...
function runBatchRequests(endpoint, infra_mode, be_mode) {
  ...
  let register_url;
  if (infra_mode === 'multi') {
    register_url = `https://${project_id}.api.antrein10.cloud/${be_mode}/queue/register?project_id=${project_id}`;
  } else if (infra_mode === 'shared') {
    register_url = `https://api.antrein10.cloud/${be_mode}/queue/register?project_id=${project_id}`;
  }
