...
fetch_infra_mode_and_be_mode() {
  ...
  while [ $retry_count -lt $max_retries ]; do
    local response=$(curl -s https://infra.antrein12.cloud)
    if [ $? -eq 0 ]; then
...
check_server_health() {
  ...
  while [ $retry_count -lt $max_retries ]; do
    local response=$(curl -s -w "\nHTTP_STATUS_CODE:%{http_code}" https://infra.antrein12.cloud)
...
BASE_URL="https://api.antrein12.cloud/${be_mode}/dashboard"
...
gather_project_urls() {
  ...
  for ((i=1; i<=num_projects; i++)); do
    ...
    project_url="https://${project_id}.antrein12.cloud/"
...
clear_projects() {
  ...
  curl -X DELETE "https://api.antrein12.cloud/${be_mode}/dashboard/project/clear" -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json"
